...
| `starter/`                              | The minimal **content-repo contract**: root `bpmiq.yml` naming the BPMN folder + README. Mirrored to `Miragon/process-documentation-starter`; the Live Host serves any repo with a root `bpmiq.yml`, nothing else.           |
| `process-documentation/`                | Example **BPM content repo** + VitePress portal (`process-documentation`) — the rich internal example consumed by validator, portal and MCP.                                                                                 |
