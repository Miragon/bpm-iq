...
> Level-4 narrative documentation. The model of record is [`order-to-cash.bpmn`](../order-to-cash.bpmn);
> facts about ownership, KPIs, and dependencies live in [`process.yaml`](../process.yaml).
...
3. **Check credit limit** — automated check against the ERP. New customers without credit terms
   must first pass through the _Customer Onboarding_ process (upstream dependency).
...
6. **Invoice Handling** (sub-process, run by the Payments & Billing Platform team) — create and
   ...
   _order cycle time_ measures order-to-payment.

## Exceptions

- **Credit check fails** → the customer is notified with the reason and offered prepayment;
  the order ends as _Order rejected_.
- **Payment not received within terms** → a timer boundary event on _Await payment_ triggers a
  reminder; the loop repeats until payment is recorded. Escalation to collections is currently
  _not modeled_ — known gap.
