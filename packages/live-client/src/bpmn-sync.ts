/**
 * Y.Text ↔ bpmn-js binding — the four sync rules from docs/platform-concept.md:
 *
 *  1. one pinned bpmn-js version across all clients (package.json)
 *  2. minimal diffs into Y.Text, never replace-all (common prefix/suffix trim)
 *  3. debounced re-import of remote changes with viewport restore and echo
...
const CANVAS_ORIGIN = "bpm-canvas";
...
export function bindBpmn(
  ...
  async function importFromY(): Promise<void> {
    ...
    try {
      ...
      if (viewbox && viewbox.width > 0 && lastExport !== "") canvas.viewbox(viewbox);
  ...
  const onCanvasChanged = async (): Promise<void> => {
    if (importing) {
  ...
  modeler.on("commandStack.changed", onCanvasChanged);
  ...
  return () => {
    ...
    modeler.off("commandStack.changed", onCanvasChanged);
    clearTimeout(timer);
  };
}
