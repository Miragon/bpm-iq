...
| `LIVE_AUTH`             | `oidc`                          | **The authentication mode ([ADR 0007](../adr/0007-idp-only-login-and-no-auth-mode.md)).** `oidc`: a login is required — the OIDC variables below (until the GitHub login is retired, `GITHUB_CLIENT_*` still counts); the server refuses to start without one. `none`: **no authentication** — every request, WebSocket join and MCP call is the local principal and every registered repository is writable. Local evaluation or a trusted single-team network only. Never inferred from missing configuration. |
